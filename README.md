...
### 2.2 编写基于OpenCV的测试程序  
经过多次验证，我们找到了使用 V4L2 作为 OpenCV 的视频捕获后端，创建管道并启动 MPV 播放器来显示热成像画面的办法。注意这里需要使用 imencode 函数将帧编码为 JPEG 格式，将编码后的帧写入管道，否则画面可能显示异常。具体的C++程序与CMake已包含在test文件夹中。只需自行交叉编译成可执行文件即可。可以使用以下命令进行编译:在项目根目录下创建一个构建目录```mkdir build && cd build``` 运行CMake来配置项目并生成构建系统```cmake ..``` 编译项目```make```  
